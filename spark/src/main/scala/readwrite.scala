...
class readwrite {
  System.setProperty("hadoop.home.dir", "C:\\hadoop\\bin\\winutil")
  ...
  def writer1(): Unit = {
    ...
    df.show()
    // 3. 写入数据集
    df.write.json("dataset/1beijing_pm.json")

    df.write.format("json").save("dataset/beijing_pm2.json1/1.json")
  }
